...
Therefore, **to prevent the reuse of the salt**, and subsequently the same encryption key, **a new key is derived each time a user logs in**. The server accomplishes this by decrypting all of the user’s data with the old key during the login process, deriving a new one with a fresh randomly generated salt, and then encrypting it with the new key. This enhances the security of the process, but leaves the salt vulnerable in the case of a database leak.
...
**User input and behavior should never be trusted**. This code pattern aims to safeguard the system against malicious user inputs, specifically SQL injection attacks and incorrect/incomplete filling of HTML forms. This is achieved by incorporating placeholders in SQL queries and implementing server-side validation.

Starting with SQL, the queries are executed by a function called [`query_db`](https://github.com/abfarias/CS50-Final-Project/blob/main/helpers.py#L25). The implementation is provided by [Flask’s website](https://flask.palletsprojects.com/en/2.3.x/patterns/sqlite3/#:~:text=def%20query_db(query%2C%20args%3D()%2C%20one%3DFalse)%3A). It is located at [`helpers.py`](/helpers.py) and takes three arguments:
- query (str): The actual SQLite3 query to be executed.
- args (tuple, optional): The values that replace the placeholders in the query
- one (bool, optional): If True, fetch only the first result. If False, fetch all results. Defaults to False.
